...
from iWebSNS_unittest_frameword.objects.edit_classification_object import *
...
class TestLogin(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.editclassification = EditClassificationPage(cls.driver)
    ...
    def test_001(self):
        ...
        self.driver.find_element(By.LINK_TEXT, '日志分类').click()
        self.driver.find_element(By.XPATH,'/html[1]/body[1]/table[1]/tbody[1]/tr[2]/td[2]/div[1]/a[1]').click()
        time.sleep(1)
        self.driver.find_element(By.XPATH,'/html[1]/body[1]/table[1]/tbody[1]/tr[3]/td[1]/input[1]').clear()
        self.driver.find_element(By.XPATH,'/html[1]/body[1]/table[1]/tbody[1]/tr[3]/td[1]/input[1]').send_keys(new_sort_name)
        time.sleep(1)
        self.driver.find_element(By.XPATH,'/html[1]/body[1]/table[1]/tbody[1]/tr[3]/td[2]/input[1]').click()
        self.driver.switch_to.default_content()
        try:
            self.assertEqual(new_sort_name,self.editclassification.get_text())
            time.sleep(1)
            print('第一个测试用例成功编辑分类')
            logger.info('验证成功编辑分类的测试用例执行Passed')
        except:
            print('第一个测试用例编辑分类失败,未进入正确的页面')
            logger.info('编辑分类失败')
    ...
    def test_002(self):
        ...
        self.driver.switch_to.default_content()
        try:
            self.assertNotEqual(new_sort_name,self.editclassification.get_text())
            time.sleep(1)
            print('第二个测试用例成功')
            logger.info('验证验证空新分类名编辑分类的测试用例执行Passed')
        except:
